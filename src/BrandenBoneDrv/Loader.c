...
NTSTATUS BBInitLdrData(IN PKLDR_DATA_TABLE_ENTRY pThisModule)
{
	...
	if (kernelBase == NULL)
	{
		DPRINT("BrandenBone: %s: Failed to retrieve Kernel base address. Aborting\n", __FUNCTION__);
	...
	{
		DPRINT("BrandenBone: %s: Failed to retrieve PsLoadedModuleList address. Aborting\n", __FUNCTION__);
		return STATUS_NOT_FOUND;
	}
...
PVOID BBGetUserModule(IN PEPROCESS pProcess, IN PUNICODE_STRING ModuleName, IN BOOLEAN isWow64)
{
	...
	__try
	{
		...
		if (isWow64)
		{
			...
			if (pPeb32 == NULL)
			{
				DPRINT("BrandenBone: %s: No PEB present. Aborting\n", __FUNCTION__);
				return NULL;
			}
			...
			{
				DPRINT("BrandenBone: %s: Loader not intialiezd, waiting\n", __FUNCTION__);
			...
			if (!pPeb32->Ldr)
			{
				DPRINT("BrandenBone: %s: Loader was not intialiezd in time. Aborting\n", __FUNCTION__);
				return NULL;
			}
...
NTSTATUS BBLookupProcessThread(IN PEPROCESS pProcess, OUT PETHREAD* ppThread)
{
	...
	if (!pInfo)
	{
		DPRINT("BrandenBone: %s: Failed to allocate memory for process list\n", __FUNCTION__);
	...
	if (NT_SUCCESS(status))
	{
		...
		DPRINT("BrandenBone: %s: Failed to locate process\n", __FUNCTION__);
...
)
{
	...
	if (NT_SUCCESS(status) && wait != FALSE)
	{
		...
		if (NT_SUCCESS(status))
		{
			...
			{
				DPRINT("BrandenBone: %s: ZwQueryInformationThread failed with status 0x%X\n", __FUNCTION__, status);
			}
		}
		else
			DPRINT("BrandenBone: %s: ZwWaitForSingleObject failed with status 0x%X\n", __FUNCTION__, status);
	}
	else
		DPRINT("BrandenBone: %s: ZwCreateThreadEx failed with status 0x%X\n", __FUNCTION__, status);
...
)
{
	...
	if (pInjectApc == NULL)
	{
		DPRINT("BrandenBone: %s: Failed to allocate APC\n", __FUNCTION__);
		return STATUS_NO_MEMORY;
	}
	...
	{
		DPRINT("BrandenBone: %s: Failed to insert APC\n", __FUNCTION__);
...
)
{
	...
	//DPRINT( "BrandenBone: %s: Called. NormalRoutine = 0x%p\n", __FUNCTION__, *NormalRoutine );
...
NTSTATUS BBMapWorker(IN PVOID pArg)
{
	...
	if (NT_SUCCESS(status))
	{
		...
		{
			DPRINT("BrandenBone: %s: Failed to allocate memory for image '%wZ'\n", __FUNCTION__, pPath);
	...
	if (NT_SUCCESS(status))
		DPRINT("BrandenBone: %s: Successfully mapped '%wZ' at 0x%p\n", __FUNCTION__, pPath, imageSection);
...
NTSTATUS BBMMapDriver(IN PUNICODE_STRING pPath)
{
	...
	if (!NT_SUCCESS(status))
	{
		DPRINT("BrandenBone: %s: Failed to create worker thread. Status: 0x%X\n", __FUNCTION__, status);
		return status;
	}
