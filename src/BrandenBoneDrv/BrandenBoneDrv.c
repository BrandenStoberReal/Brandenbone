...
NTSTATUS DriverEntry(IN PDRIVER_OBJECT DriverObject, IN PUNICODE_STRING RegistryPath)
{
	...
	if (!NT_SUCCESS(status))
	{
		if (status == STATUS_NOT_SUPPORTED)
			DPRINT("BrandenBone: %s: Unsupported OS version. Aborting\n", __FUNCTION__);
	...
	if (!NT_SUCCESS(status))
	{
		DPRINT("BrandenBone: %s: Failed to setup notify routine with staus 0x%X\n", __FUNCTION__, status);
		return status;
	}
	...
	if (!NT_SUCCESS(status))
	{
		DPRINT("BrandenBone: %s: IoCreateSymbolicLink failed with status 0x%X\n", __FUNCTION__, status);
...
NTSTATUS BBGetBuildNO(OUT PULONG pBuildNo)
{
	...
	if (NT_SUCCESS(status))
	{
		...
		DPRINT("BrandenBone: %s: ZwOpenKey failed with status 0x%X\n", __FUNCTION__, status);
...
NTSTATUS BBInitDynamicData(IN OUT PDYNAMIC_DATA pData)
{
	...
	if (status == STATUS_SUCCESS)
	{
		...
		DPRINT(
			"BrandenBone: OS version %d.%d.%d.%d.%d - 0x%x\n",
		...
		DPRINT(
			"BrandenBone: Dynamic search status: SSDT - %s, ExRemoveTable - %s\n",
		...
		{
			DPRINT(
				"BrandenBone: %s: g_KdBlock->KernBase: %p, GetKernelBase() = 0x%p \n",
