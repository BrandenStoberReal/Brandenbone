...
namespace BrandenBone
{
	...
	{
	public:
		BRANDENBONE_API MMap(class Process& proc);
		BRANDENBONE_API ~MMap(void);
		...
		BRANDENBONE_API call_result_t<ModuleDataPtr> MapImage(
		...
		BRANDENBONE_API NTSTATUS UnmapAllModules();

		/// <summary>
		/// Remove any traces from remote process
		/// </summary>
		/// <returns></returns>
		BRANDENBONE_API void Cleanup();

		/// <summary>
		/// Reset local data
		/// </summary>
		BRANDENBONE_API inline void reset() { _images.clear(); _pAContext.Reset(); _usedBlocks.clear(); }
	private:
		/// <summary>
		/// Manually map PE image into underlying target process
		/// </summary>
