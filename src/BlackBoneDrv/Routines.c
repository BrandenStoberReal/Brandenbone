...
NTSTATUS BBDisableDEP(IN PDISABLE_DEP pData)
{
	...
	if (NT_SUCCESS(status))
	{
		...
		{
			DPRINT("BrandenBone: %s: Invalid _KEXECUTE_OPTIONS offset\n", __FUNCTION__);
...
NTSTATUS BBSetProtection(IN PSET_PROC_PROTECTION pProtection)
{
	...
	if (NT_SUCCESS(status))
	{
		...
		{
			DPRINT("BrandenBone: %s: Invalid protection flag offset\n", __FUNCTION__);
			status = STATUS_INVALID_ADDRESS;
		}
	}
	else
	...
	if (EnumParameter != NULL)
	{
		...
		if (Handle == (HANDLE)pAccess->handle)
		{
			if (ExpIsValidObjectEntry(HandleTableEntry))
			{
				...
				DPRINT("BrandenBone: %s: 0x%X:0x%X handle is invalid\n. HandleEntry = 0x%p",
...
NTSTATUS BBGrantAccess(IN PHANDLE_GRANT_ACCESS pAccess)
{
	...
	{
		DPRINT("BrandenBone: %s: Invalid ObjTable address\n", __FUNCTION__);
...
NTSTATUS BBUnlinkHandleTable(IN PUNLINK_HTABLE pUnlink)
{
	...
	{
		DPRINT("BrandenBone: %s: Invalid ExRemoveTable/ObjTable address\n", __FUNCTION__);
		return STATUS_INVALID_ADDRESS;
	}
	...
	{
		DPRINT("BrandenBone: %s: Unsupported kernel build version\n", __FUNCTION__);
	...
	if (NT_SUCCESS(status))
	{
		...
		//DPRINT( "BrandenBone: %s: ExRemoveHandleTable address 0x%p. Object Table offset: 0x%X\n",
...
NTSTATUS BBAllocateFreePhysical(IN PEPROCESS pProcess, IN PALLOCATE_FREE_MEMORY pAllocFree, OUT PALLOCATE_FREE_MEMORY_RESULT pResult)
{
	...
	{
		DPRINT("BrandenBone: %s: Region size if too big: 0x%p\n", __FUNCTION__, pAllocFree->size);
		return STATUS_INVALID_PARAMETER;
	}
	...
	if (pAllocFree->allocate != FALSE)
	{
		...
		pRegionBase = ExAllocatePool2(POOL_FLAG_NON_PAGED, pAllocFree->size, BB_POOL_TAG);
		if (!pRegionBase)
		...
		if (pResult->address)
		{
			...
			if (pEntry == NULL)
			{
				pEntry = ExAllocatePool2(POOL_FLAG_PAGED, sizeof(MEM_PHYS_PROCESS_ENTRY), BB_POOL_TAG);
			...
			pMemEntry = ExAllocatePool2(POOL_FLAG_PAGED, sizeof(MEM_PHYS_ENTRY), BB_POOL_TAG);
